...
## <img src="https://github.com/lipis/flag-icons/raw/refs/heads/main/flags/4x3/in.svg" width="30"> ZapUnlocked-API ???? ???

WhatsApp API ????? ????? ????? ??????? ???? ??: ??????? ?? ??????? ????? ????? ???, ????? ??????, ????? ?????? ????? ?? ????? ???? ?? ??????? ?? ?????? ???? ????? **ZapUnlocked-API ??? ????? ?? ??? ???**
...
**1.** [Alwaysdata.com](https://www.alwaysdata.com/) ?? ???? ????? · **?????** ??????

**2.** SSH `https://ssh-[??????????].alwaysdata.net` ?? ????????
...
**5.** **Advanced · Services · Add a service** ??? ???? (24/7) ????????? ????:
...
```
http://services-[??????????].alwaysdata.net:8300/
```

> [!TIP]
> URL ????? ??? ?? ???? ?? ?? ???? ??? *(????????)* ????? ????? ?? ????? ???? ?? ??? **Web · Sites · Add a site** ??? `http://[??????????].alwaysdata.net` ?? ????? ???? ??? **?????? ???????? (Reverse Proxy)** ????????? ?????
...
```text
http://services-[??????????].alwaysdata.net:8300/qr?API_KEY=????_?????_???????
```
