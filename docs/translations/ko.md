...
## <img src="https://github.com/lipis/flag-icons/raw/refs/heads/main/flags/4x3/kr.svg" width="30"> ZapUnlocked-API??

WhatsApp API ??? ?? ???? ????? ??? ?????: ? ?? ???? ?? ??, ??? ??, ??? ??, ?3? ??? ???? ???. **ZapUnlocked-API? ?? ??? ?? ?????.**
...
**2.** SSH `https://ssh-[kullanici].alwaysdata.net`? ??.
...
> [!TIP]
> URL? ???? ?? ?? ?????. *(?? ??)* ??? ?? ???? ????? **Web · Sites · Add a site**?? `http://[kullanici].alwaysdata.net`? ???? **??? ??? (Reverse Proxy)** ? ?????.
...
```text
http://services-[kullanici].alwaysdata.net:8300/qr?API_KEY=??_????
```
